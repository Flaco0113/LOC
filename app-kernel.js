...
export function createKernel(initial, {save=()=>{}, hosted=false, mailConfig={mode:'disabled',enabled:false}, dataDir=''}={}) {
let db=initial;
...
function persist() { db.revision++; save(db); }
...
function advanceClocks() {
  ...
  transaction(() => due.forEach(l => {
    while(l.status==='live'&&l.deadline<=now()) {
...
function session(res,u,remember) { for(const [key,value] of Object.entries(db.sessions))if(value.expires<=now())delete db.sessions[key]; const token=crypto.randomBytes(32).toString('hex'); db.sessions[hash(token)]={userId:u.id,expires:now()+(remember?30:1)*86400000}; res.setHeader('Set-Cookie',`loc_session=${token}; HttpOnly; SameSite=Strict; Path=/${hosted?'; Secure':''}${remember?'; Max-Age=2592000':''}`); }
...
function api(req,res,pathname,b,u) {
  ...
  requireThat(b && typeof b==='object' && !Array.isArray(b),'Invalid request.');
  ...
  if(pathname==='/api/join') { rateLimit(req); return transaction(()=>{
    ...
    if(member(l,u.id))return {...snapshot(u),joinedId:l.id};
    requireThat(l.status==='scheduled'&&!l.locked&&l.competitionState==='active','This league is not accepting managers.','code');
    requireThat(l.members.length<l.capacity,'This league is full. Ask the commissioner for a place.','code');
    l.members.push({id:u.id,name:u.name});activity(l,u.id,'Joined using the league code.');
    return {...snapshot(u),joinedId:l.id};
  }); }
  ...
  return transaction(()=>{
    ...
    if(action==='pick') pick(l,u.id,b.teamId,b.version,b.override ? clean(b.reason,500):null);
      ...
      commissioner(l,u.id);
      if(action==='joincode') {
        requireThat(!l.sample,'Create a personal league to invite friends.');
        requireThat(['generate','revoke'].includes(b.action),'Choose generate or revoke.');
      ...
      else if(action==='settings') {requireThat(clean(b.name).length>=3,'Use at least 3 characters.','name');l.name=clean(b.name);l.description=clean(b.description,500);l.locked=!!b.locked;
        ...
        requireThat(!hosted||mailConfig.enabled,'Share a league join code from Settings. Email-address invitations require verified email delivery.');
...
return {api, readSession, advanceClocks, flushMail, getState:()=>db, exportLeague(req,url) {
 const user=readSession(req);if(!user)fail(401,'Sign in to download a league export.');
 const l=leagueFor(user.id,url.pathname.split('/')[3]),format=url.searchParams.get('format');
